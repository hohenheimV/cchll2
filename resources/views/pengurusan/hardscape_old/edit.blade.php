@extends('layouts.pengurusan.app')

@section('title', 'Kemaskini Hardscape')

@section('content')
{!! Form::model($hardscape, ['route' => ['pengurusan.hardscape.update', $hardscape], 'method'=>'PUT','id'=>'modalFormHardscape']) !!}
<div class="modal-header card-olive card-outline d-flex p-2">
    <div class="flex-grow-1">
        <h5 class="my-1 mx-2 text-uppercase">@yield('title')</h5>
    </div>
</div>
<!-- /.modal-header -->
<div class="modal-body">
    @include('pengurusan.hardscape._form')
</div>
<!-- /.modal-body -->
<div class="modal-footer">
    {{ Form::button('Batal', ['class'=>'btn btn-secondary','data-dismiss'=>'modal']) }}
    {!! Form::button('<i class="fas fa-save"></i> Kemaskini', ['class'=>'btn btn-success','type'=>'submit']) !!}
</div>
<!-- /.modal-footer -->
{!! Form::close() !!}
@endsection
