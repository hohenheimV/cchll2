                    ...
                    <div class="card-tools">
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group" role="group" aria-label="First group">
                                ...
                                ['onclick'=>"window.location='".route('pengurusan.panorama.create')."'",
                                'class'=>'btn bg-success btn-sm', Html::tooltip('Daftar Panorama')
                                ]) !!}
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table id="example" class="responsive table table-bordered table-hover table-striped mb-0">
                            ...
                            <tbody>
                                @php($index = $panoramas->firstItem())
                                @forelse($panoramas as $panorama)
                                <tr>
                                    <td>{{ $index++ }}</td>
                                    <td class="p-0">
                                        {!! '<img class="image-thumb p-1 w-75 mx-auto d-block embed-responsive-item" alt="Gambar panorama"
                                            src="'.asset($panorama->gambar_360 ? 'storage/images/shares/panorama/'.$panorama->gambar_360 : 'img/no-photos.png').'">' !!}
                                    </td>
                                    <td>{{ $panorama->tajuk }}</td>
                                    <td>{{ \Str::words($panorama->keterangan, 10,'....') }} </td>
                                    <td class="text-center">{!! Html::datetime($panorama->tarikh,'d-m-Y') !!}</td>
                                    <td class="text-center">{!! Html::datetime($panorama->created_at,'d-m-Y') !!}</td>
                                    <td class="text-center">{!! Html::datetime($panorama->updated_at,'d-m-Y') !!}</td>
                                    <td>
                                        <div class="btn-group">


                                            {!! Form::button('<i class="fas fa-search"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.panorama.show',$panorama)."'",
                                            'class'=>'btn bg-info btn-sm', Html::tooltip('Butiran Panorama')
                                            ]) !!}

                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.panorama.edit',$panorama)."'",
                                            'class'=>'btn bg-warning btn-sm', Html::tooltip('Kemaskini Panorama')
                                            ]) !!}


                                            {!! Form::button('<i class="fas fa-trash"></i>', ['class'=>'btn btn-danger btn-sm',
                                            'data-url'=>route('pengurusan.panorama.destroy',$panorama),
                                            'data-text'=>'Jawatan : '.$panorama->tajuk,
                                ...
                                @empty
                                {!! Html::forelse_alert(request('keyword'),'Panorama') !!}
                                @endforelse
                            </tbody>
                        </table>
                    </div>
