...
@section('card-title', 'Maklumat Batang Pokok')
                ...
                <div class="card-body p-0">
                    <div class="d-flex px-1">
                        ...
                        <div class="p-1">
                            ...
                            ['onclick'=>"window.location='".route('pengurusan.softscapes.batang.create',$softscape)."'",'class'=>'btn bg-primary btn-sm']) !!}
                        </div>
                    </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-fixed table-sm table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center w-15 align-middle">Bentuk Batang Pokok</th>
                                    <th class="text-center w-15 align-middle">Ketinggian Batang Pokok</th>
                                    <th class="text-center w-15 align-middle">Diameter Batang</th>
                                    <th class="text-center w-15 align-middle">Tekstur Batang</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{!! $data->ketinggian ?? $null !!}</td>
                                    <td class="text-center">{!! $data->diameter ?? $null !!}</td>
                                    <td class="text-center">{!! $data->tekstur ?? $null !!}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.batang.show',$data)."'",'class'=>'btn bg-info btn-sm']) !!}
                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.batang.edit',$data)."'",'class'=>'btn bg-warning btn-sm']) !!}
                                            {{--  button delete  --}}
                                ...
                                @empty
                                {!! Html::forelse_alert(request('keyword'),'landskap lembut berkaitan dengan bahagian batang') !!}
                                @endforelse
                            </tbody>
                        </table>
                    </div>
...
@section('modal')
<div class="modal fade" id="modalMaklumatBatang" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalMaklumatBatangLabel" aria-hidden="true">
...
<!-- Modal HTML -->
<div class="modal" id="modalDelMaklumatBatang" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modalDaerahLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            {!! Form::open(['method'=>'DELETE','id'=>'modalFormDelMaklumatBatang']) !!}
...
<script>
    $(document).ready(function () {
        ...
        $('#modalDelMaklumatBatang').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);        // Button that triggered the modal
            var id = button.data('id'); // Extract info from data-* attributes
            var url = '{{ route("pengurusan.softscapes.batang.destroy", ":id") }}'.replace(':id', id);
            $('#modalFormDelMaklumatBatang').attr('action', url);
        });


        $('#modalMaklumatBatang').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var href = button.data('href'); // Extract info from data-* attributes
            $('[data-tooltip="tooltip"]').tooltip('hide');
            // Load URL from data-href
            $('#modalMaklumatBatang .modal-content').load(href, function (responseTxt, statusTxt, xhr) {
                ...
                if (statusTxt == "success") {
                    $('#modalMaklumatBatang').modal('show'); // Show Modal start
                    // clear modal content if modal closed
                    $('#modalMaklumatBatang').on('hidden.bs.modal', function () {
                        $('[data-tooltip="tooltip"]').tooltip('hide');
        ...
        function validation() {
            ...
            $('#modalFormBatang').validate({ //sets up the validator
