...
@section('card-title', 'Maklumat Penyelenggaraan Pembajaan')
                ...
                <div class="card-body p-0">
                    <div class="d-flex px-1">
                        ...
                        <div class="p-1">
                            ...
                            ['onclick'=>"window.location='".route('pengurusan.softscapes.pembajaan.create',$softscape)."'",'class'=>'btn bg-primary btn-sm']) !!}
                        </div>
                    </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-fixed table-sm table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center w-15 align-middle">Jenis Baja</th>
                                    <th class="text-center w-15 align-middle">Kaedah Baja</th>
                                    <th class="text-center w-9 align-middle">Tarikh Baja</th>
                            ...
                            <tbody>
                                ...
                                @php($index = $pembajaan->firstItem())
                                @forelse($pembajaan as $data)
                                <tr>
                                    <th scope="row">{{ $index++ }}</th>
                                    <td class="text-center">{!! $data->created_at ? '<h5>
                                            <span class="badge badge-primary">'.Html::datetime($data->tarikh,'Y').'</span>
                                        </h5>' : $null !!}</td>
                                    <td class="text-center">{!! $data->jenis ?? $null !!}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.pembajaan.show',$data)."'",'class'=>'btn bg-info btn-sm']) !!}
                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.pembajaan.edit',$data)."'",'class'=>'btn bg-warning btn-sm']) !!}
                                            {{--  button delete  --}}
...
@section('modal')
<div class="modal fade" id="modalMaklumatPembajaan" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalMaklumatPembajaanLabel" aria-hidden="true">
...
<!-- /.modal -->
<!-- Modal HTML -->
<div class="modal" id="modalDelMaklumatPembajaan" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalDaerahLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            {!! Form::open(['method'=>'DELETE','id'=>'modalFormDelMaklumatPembajaan']) !!}
...
<script>
    $(document).ready(function () {
        ...
        $('#modalDelMaklumatPembajaan').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);        // Button that triggered the modal
            var id = button.data('id'); // Extract info from data-* attributes
            var url = '{{ route("pengurusan.softscapes.pembajaan.destroy", ":id") }}'.replace(':id', id);
            $('#modalFormDelMaklumatPembajaan').attr('action', url);
        });

        $('#modalMaklumatPembajaan').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var href = button.data('href'); // Extract info from data-* attributes
            $('[data-tooltip="tooltip"]').tooltip('hide');
            // Load URL from data-href
            $('#modalMaklumatPembajaan .modal-content').load(href, function (responseTxt, statusTxt, xhr) {
                ...
                if (statusTxt == "success") {
                    $('#modalMaklumatPembajaan').modal('show'); // Show Modal start
                    // clear modal content if modal closed
                    $('#modalMaklumatPembajaan').on('hidden.bs.modal', function () {
                        $('[data-tooltip="tooltip"]').tooltip('hide');
        ...
        function validation() {
            ...
            $('#modalFormPembajaan').validate({ //sets up the validator
