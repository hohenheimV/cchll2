                    ...
                    <div class="card-tools">
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            ...
                            <div class="btn-group" role="group" aria-label="First group">
                                ...
                                'onclick'=>"window.location='".route('pengurusan.feedbacks.create')."'",
                                Html::tooltip('Daftar')
                                ]) !!}
                            </div>
                        </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-striped table-sm mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center align-middle">Mesej</th>
                            ...
                            <tbody>
                                ...
                                @php($index = $feedbacks->firstItem())
                                @forelse($feedbacks as $feedback)
                                <tr>
                                    <td>{{ $index++ }}</td>
                                    <td>
                                        <span class="badge badge-dark">
                                            {!! $feedback->ref_num ?? $null !!}
                                        </span>
                                    </td>
                                    <td class="text-center">{!! $feedback->created_at->format('d-m-Y') !!}</td>
                                    <td>{!! $feedback->name.'<br />'.$feedback->email !!}</td>
                                    <td>{{ Str::limit($feedback->message, 250) }}</td>
                                    <td>{{ $feedback->status }}</td>
                                    <td>
                                        <div class="btn-group">
                                            {!! Form::button('<i class="fas fa-search"></i>', [
                                            'class'=>'btn btn-info btn-sm',
                                            'onclick'=>"window.location='".route('pengurusan.feedbacks.show',$feedback)."'"
                                            ]) !!}
                                            @can('feedback-edit')
                                            ...
                                            btn-sm',
                                            'data-url'=>route('pengurusan.feedbacks.destroy',$feedback->id),
