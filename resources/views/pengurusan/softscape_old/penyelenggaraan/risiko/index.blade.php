...
@section('card-title', 'Maklumat Penyelenggaraan Risiko')
                ...
                <div class="card-body p-0">
                    <div class="d-flex px-1">
                        ...
                        <div class="p-1">
                            ...
                            ['onclick'=>"window.location='".route('pengurusan.softscapes.risiko.create',$softscape)."'",'class'=>'btn bg-primary btn-sm']) !!}
                        </div>
                    </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-fixed table-sm table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center w-15 align-middle">Jenis Risiko</th>
                                    <th class="text-center w-15 align-middle">Kaedah Risiko</th>
                                    <th class="text-center w-9 align-middle">Tahap Risiko</th>
                            ...
                            <tbody>
                                ...
                                @php($index = $risiko->firstItem())
                                @forelse($risiko as $data)
                                <tr>
                                    ...
                                    <td class="text-center">{!! $data->tahap ?? $null !!}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.risiko.show',$data)."'",'class'=>'btn bg-info btn-sm']) !!}
                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.risiko.edit',$data)."'",'class'=>'btn bg-warning btn-sm']) !!}
                                            {{--  button delete  --}}
...
@section('modal')
<div class="modal fade" id="modalMaklumatRisiko" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalMaklumatRisikoLabel" aria-hidden="true">
...
<!-- Modal HTML -->
<div class="modal" id="modalDelMaklumatRisiko" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modalDaerahLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            {!! Form::open(['method'=>'DELETE','id'=>'modalFormDelMaklumatRisiko']) !!}
...
<script>
    $(document).ready(function () {
        ...
        $('#modalDelMaklumatRisiko').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);        // Button that triggered the modal
            var id = button.data('id'); // Extract info from data-* attributes
            var url = '{{ route("pengurusan.softscapes.risiko.destroy", ":id") }}'.replace(':id', id);
            $('#modalFormDelMaklumatRisiko').attr('action', url);
        });

        $('#modalMaklumatRisiko').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var href = button.data('href'); // Extract info from data-* attributes
            $('[data-tooltip="tooltip"]').tooltip('hide');
            // Load URL from data-href
            $('#modalMaklumatRisiko .modal-content').load(href, function (responseTxt, statusTxt, xhr) {
                ...
                if (statusTxt == "success") {
                    $('#modalMaklumatRisiko').modal('show'); // Show Modal start
                    // clear modal content if modal closed
                    $('#modalMaklumatRisiko').on('hidden.bs.modal', function () {
                        $('[data-tooltip="tooltip"]').tooltip('hide');
        ...
        function validation() {
            ...
            $('#modalFormRisiko').validate({ //sets up the validator
