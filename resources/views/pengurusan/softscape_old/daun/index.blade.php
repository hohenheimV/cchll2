...
@section('card-title', 'Maklumat Daun')
                ...
                <div class="card-body p-0">
                    <div class="d-flex px-1">
                        ...
                        <div class="p-1">
                            ...
                            ['onclick'=>"window.location='".route('pengurusan.softscapes.daun.create',$softscape)."'",'class'=>'btn bg-primary btn-sm']) !!}
                        </div>
                    </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-fixed table-sm table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center w-15 align-middle">Warna Daun</th>
                                    <th class="text-center w-15 align-middle">Bentuk Daun</th>
                                    <th class="text-center w-15 align-middle">Cara Percambahan Daun</th>
                                    <th class="text-center w-15 align-middle">Jenis Daun</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{!! $data->percambahan ?? $null !!}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.daun.show',$data)."'",'class'=>'btn bg-info btn-sm']) !!}
                                            {{--  button edit  --}}
                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.daun.edit',$data)."'",'class'=>'btn bg-warning btn-sm']) !!}
                                            {{--  button delete  --}}
                                ...
                                @empty
                                {!! Html::forelse_alert(request('keyword'),'landskap lembut berkaitan dengan bahagian daun') !!}
                                @endforelse
                            </tbody>
                        </table>
                    </div>
...
@section('modal')
<div class="modal fade" id="modalMaklumatDaun" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalMaklumatDaunLabel" aria-hidden="true">
...
<!-- /.modal -->
<!-- Modal HTML -->
<div class="modal" id="modalDelMaklumatDaun" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modalDaerahLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            {!! Form::open(['method'=>'DELETE','id'=>'modalFormDelMaklumatDaun']) !!}
...
<script>
    $(document).ready(function () {
        ...
        $('#modalDelMaklumatDaun').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);        // Button that triggered the modal
            var id = button.data('id'); // Extract info from data-* attributes
            var url = '{{ route("pengurusan.softscapes.daun.destroy", ":id") }}'.replace(':id', id);
            $('#modalFormDelMaklumatDaun').attr('action', url);
        });

        $('#modalMaklumatDaun').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var href = button.data('href'); // Extract info from data-* attributes
            $('[data-tooltip="tooltip"]').tooltip('hide');
            // Load URL from data-href
            $('#modalMaklumatDaun .modal-content').load(href, function (responseTxt, statusTxt, xhr) {
                ...
                if (statusTxt == "success") {
                    $('#modalMaklumatDaun').modal('show'); // Show Modal start
                    // clear modal content if modal closed
                    $('#modalMaklumatDaun').on('hidden.bs.modal', function () {
                        $('[data-tooltip="tooltip"]').tooltip('hide');
        ...
        function validation() {
            ...
            $('#modalFormDaun').validate({ //sets up the validator
