...
@section('card-title', 'Maklumat Buah')
                ...
                <div class="card-body p-0">
                    <div class="d-flex px-1">
                        ...
                        <div class="p-1">
                            ...
                            ['onclick'=>"window.location='".route('pengurusan.softscapes.buah.create',$softscape)."'",'class'=>'btn bg-primary btn-sm']) !!}
                        </div>
                    </div>
                        ...
                        <table id="example" class="responsive table table-bordered table-hover table-fixed table-sm table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th class="text-center w-15 align-middle">Warna Buah</th>
                                    <th class="text-center w-15 align-middle">Bentuk Buah</th>
                                    <th class="text-center w-15 align-middle">Saiz Buah</th>
                                    <th class="text-center w-15 align-middle">Musim Buah</th>
                                    <th class="text-center w-15 align-middle">Tempoh Buah</th>
                            ...
                            <tbody>
                                ...
                                @php($index = $buah->firstItem())
                                @forelse($buah as $data)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.buah.show',$data)."'",'class'=>'btn bg-info btn-sm']) !!}
                                            {!! Form::button('<i class="fas fa-pencil-alt"></i>',
                                            ['onclick'=>"window.location='".route('pengurusan.softscapes.buah.edit',$data)."'",'class'=>'btn bg-warning btn-sm']) !!}
                                            {{--  button delete  --}}
...
@section('modal')
<div class="modal fade" id="modalMaklumatBuah" data-backdrop="static" data-keyboard="false" role="dialog"
    aria-labelledby="modalMaklumatBuahLabel" aria-hidden="true">
...
<!-- Modal HTML -->
<div class="modal" id="modalDelMaklumatBuah" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="modalDaerahLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            {!! Form::open(['method'=>'DELETE','id'=>'modalFormDelMaklumatBuah']) !!}
...
<script>
    $(document).ready(function () {
        ...
        $('#modalDelMaklumatBuah').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget);        // Button that triggered the modal
            var id = button.data('id'); // Extract info from data-* attributes
            var url = '{{ route("pengurusan.softscapes.buah.destroy", ":id") }}'.replace(':id', id);
            $('#modalFormDelMaklumatBuah').attr('action', url);
        });

        $('#modalMaklumatBuah').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var href = button.data('href'); // Extract info from data-* attributes
            $('[data-tooltip="tooltip"]').tooltip('hide');
            // Load URL from data-href
            $('#modalMaklumatBuah .modal-content').load(href, function (responseTxt, statusTxt, xhr) {
                ...
                if (statusTxt == "success") {
                    $('#modalMaklumatBuah').modal('show'); // Show Modal start
                    // clear modal content if modal closed
                    $('#modalMaklumatBuah').on('hidden.bs.modal', function () {
                        $('[data-tooltip="tooltip"]').tooltip('hide');
        ...
        function validation() {
            ...
            $('#modalFormBuah').validate({ //sets up the validator
