...
@section('content')
    ...
    'onclick' => "window.location='" . route('pengurusan.hardscape.edit', $hardscape) . "'",
                    ...
                    <div class="card card-olive card-outline card-outline-tabs">
                                ...
                                @foreach (range(date('Y'), 2021) as $yr)
                                    <li class="nav-item">
                                        <a class="nav-link {{ request()->has('tahun') && request()->get('tahun') == $yr ? 'active' : (!request()->has('tahun') && date('Y') == $yr ? 'active' : '') }}"
                                            href="{{ route('pengurusan.hardscape.show', [$hardscape, 'tahun' => $yr]) }}">
                                            {{ $yr }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="card-body">
							<div class="tab-content table-hardscape text-sm">
								@include('pengurusan.hardscape.show._maklumat')
								@include('pengurusan.hardscape.show._gambar')
								{{-- @include('pengurusan.hardscape.show._penyelenggaraan') --}}
							</div>
                        </div>
                    </div>
